        ...
        for (int j = 0; j < settings.width; j++) {
            if (map[i][j].isRevealed) {
                if (map[i][j].isMine) {
                    ...
                    printf("|●| ");
                    resetColor();
                } else if (map[i][j].surroundingMines > 0) {    //가려진 곳을 열람
                    switch (map[i][j].surroundingMines) {       //주의에 몇개의 지뢰가 있는가???
                        case 1:
...
void startGame(GameSettings settings, int hardcoreMode) {
    ...
    while (lives > 0) {
        ...
        if (clearedCells == totalCells || remainingMines == 0 || allMinesFound(map, settings)) {
            ...
            stage++;
            int newMines = settings.mines + (settings.mines * 0.25);        //클리어 마다 지뢰 매설수 25%증가
            if ((newMines % 2) != 0) newMines++;
            if (newMines <= (settings.width * settings.height) * 0.7) {       //지뢰는 맵의 70퍼센트 이상을 차지 할 수 없음.
...
void explainRules() {
    ...
    printf("\n-탐색하는 칸 반경 1칸 내 지뢰가 몇 개 있는지 알려주는 숫자 인디케이터-\n");
    ...
    printf("\n맵 탐색 :\n");
    printf("S(s) == 탐색 , F(f) == 깃발, out == 나가기\n");
    printf("깃발이 세워진 위치에 다시 깃발을 세우라는 명령을 하면 깃발을 회수\n");
    printf("맵 내에 모든 지뢰를 찾으면 자동으로 스테이지 클리어\n");
    printf("스테이지 클리어시 목숨 1 지급\n");
    printf("스테이지 클리어시 맵 크기는 가로 세로 2만큼 맵 크기 증가\n");
    printf("스테이지 클리어시 지뢰의 수는 이전 맵의 지뢰 숫자의 125%% 로 증가,\n");
...
void displayCredits() {
    ...
    printf("\n V1.5 : 최종 코드 안정화, 지뢰매설 버그 수정");
    printf("\n V2.0 : 최종버전. 게임 룰 설명 추가 및 밸런스 패치");
